...
def afficherLesSalles():
    ...
    """
    rowsSalles =db(db.salle.categorie_id==db.categorie.id).select(db.salle.id,db.salle.nom,db.categorie.nom, db.categorie.heureOuverture, db.categorie.heureOuvertureMinutes, db.categorie.heureFermeture, db.categorie.heureFermetureMinutes, db.salle.capacite, orderby=(db.categorie.nom,db.salle.capacite)) #requête permettant de récupérer les informations des salles
